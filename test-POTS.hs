...
-- main = initializeAndRun (createControlProcsBusy)
main = initializeAndRun (createControlProcs)

{-
In the first test, we call our own number; we should get a busy tone
...
-- This is run by: test-POTS.exe -cfgRole=NODEA
...
-- Meanwhile, elsewehere (but at the same IP address) run: 
-- test-POTS.exe -cfgRole=NODEB
